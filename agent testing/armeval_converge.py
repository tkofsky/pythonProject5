...
- Convergence stats per arm in a separate CSV
- Arm abandonment events logged when an arm is pruned early
"""
...
CSV_PATH_TRIALS = "shot_bandit_log_ucb_trials.csv"
CSV_PATH_SUMMARY = "shot_bandit_convergence_summary.csv"
CSV_PATH_ABANDON = "shot_bandit_abandon_events.csv"
...
# Early-abandon logic
MIN_PULLS_FOR_EVAL = 10      # minimum pulls before considering abandonment
ABANDON_MARGIN = 0.0         # margin for UCB < best LCB - margin
CONV_WIDTH_THRESHOLD = 0.20  # width of UCB band for convergence heuristic
...
def select_arm_ucb1(
    ...
    if not available:
        # Fallback if everything was abandoned
...
def check_and_mark_abandon(
    ...
    """
    Returns list of arms newly abandoned at this trial.
    """
    n_arms = len(q_values)
...
def main():
    ...
    for t in range(N_TRIALS):
        ...
        for i in newly_abandoned:
            best_idx = max(
                [j for j in range(n_arms) if not abandoned[j] or j == i and counts[j] > 0],
                key=lambda j: q_values[j]
            )
            ...
            print(
                f"[ABANDON] trial={t} arm={i} shots={SHOT_VALUES[i]} "
    ...
    for i, shots in enumerate(SHOT_VALUES):
        print(
            f"  shots={shots}: Q={q_values[i]:.3f}, "
            f"n={counts[i]}, abandoned={abandoned[i]}, "
            f"abandon_trial={abandon_trial[i]}"
        )
