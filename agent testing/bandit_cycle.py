"""
functional_prompt_bandit_ucb1_csv_dataset.py

UCB1 bandit over prompt templates, cycling through examples in agent_dataset.json,
with CSV logging, timestamps, and latency.
"""
...
CSV_LOG      = "bandit_log_ucb1_dataset.csv"
...
def get_example_text(example: Dict[str, Any]) -> str:
    for key in ("input", "user_input", "prompt", "text"):
        if key in example and isinstance(example[key], str):
            return example[key]
    # fallback: stringify entire example
...
def main():
    ...
    for t in range(n_trials):
        ...
        example_text = get_example_text(example)
