# %%
# bandit_fewshot_agent_router_hybrid.py
#
...
CSV_LOG     = "bandit_fewshot_agent_log_router_hybrid.csv"
...
def build_pass2_messages(pass1_json: dict, temperature: float) -> List[dict]:
    # Strict Pass-2 template (anchored)
    sys = (
        "You are an agent planner. You will be given a JSON object with keys: "
        "intent, entities, constraints, urgency. Using only these fields, output JSON "
        "with a single key 'steps' = an array of 3-5 short, imperative verb phrases.\n"
        "Anchor steps to the entities/constraints (mention locations, dates, amounts). "
        "Do NOT invent new facts. Output JSON only, no commentary."
    )
...
def main():
    ...
    for itr in range(1, ITERATIONS+1):
        ...
        if use_two_pass:
            ...
            out1, toks1 = call_llm(msgs1, arm.temperature)
            j1 = safe_json(out1)
            pass1_valid = isinstance(j1, dict) and all(k in j1 for k in ["intent","entities","constraints","urgency"])

            if pass1_valid:
                # Pass-2: steps-only, anchored
                msgs2 = build_pass2_messages(j1, arm.temperature)
                out2, toks2 = call_llm(msgs2, arm.temperature)
                j2 = safe_json(out2)
                if isinstance(j2, dict) and "steps" in j2:
                    # merge
                    final = {**j1, "steps": j2["steps"]}
                    ...
                    output_text, toks = call_llm(msgs, arm.temperature)
                    two_pass_flag = 0
            else:
        ...
        if itr % 10 == 0:
            ...
            print(f"[iter {itr}] best_avg≈{best_avg:.3f} | router(two_pass%≈{100*sum(1 for k in range(itr) if k)/max(1,itr):.1f})")
