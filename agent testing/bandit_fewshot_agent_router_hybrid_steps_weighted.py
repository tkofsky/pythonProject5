# Create a new runner that adds reward shaping (higher weight for F1_steps) and keeps n-best reranking.
# File: /mnt/data/bandit_fewshot_agent_router_hybrid_steps_weighted.py
#
from pathlib import Path

code = r""
# bandit_fewshot_agent_router_hybrid_steps_weighted.py
#
# Router + Hybrid Two-Pass bandit with STEP-WEIGHTED reward shaping
# - Same as '..._steps_patch.py' but adds reward shaping to upweight F1_steps
# - Logs both unshaped reward and shaped reward
#
# Usage:
#   export OPENAI_API_KEY=YOUR_KEY
#   python bandit_fewshot_agent_router_hybrid_steps_weighted.py
#
# Inputs:
#   agent_dataset.json
...
#
# Output:
#   bandit_fewshot_agent_log_router_hybrid_steps_weighted.csv
#
...
TEMPS            = [0.2, 0.5]        # decoding temperatures
...
STEP_WEIGHT             = 2.0        # upweight F1_steps in shaped reward
BONUS_ALL_FIELDS        = 0.05       # same presence bonus as baseline
...
# ---- Pass-2 messages: anchored + canonical ----
def build_pass2_messages(pass1_json: dict) -> List[dict]:
...
# ---- Step normalization & generic blocker ----
CANON_MAP = {
...
def too_generic(steps):
    ...
    return any(re.search(p, txt) for p in GENERIC_PATTERNS)
...
def score_fields(pred: dict, ref: dict):
    ...
    f_intent      = f1(tokens(pred.get("intent")),      tokens(ref.get("intent")))
    f_entities    = f1(tokens(pred.get("entities")),    tokens(ref.get("entities")))
    f_constraints = f1(tokens(pred.get("constraints")), tokens(ref.get("constraints")))
    f_urgency     = f1(tokens(pred.get("urgency")),     tokens(ref.get("urgency")))
    f_steps       = f1(tokens(pred.get("steps")),       tokens(ref.get("steps")))
    return f_intent, f_entities, f_constraints, f_urgency, f_steps

def base_reward_from_fields(f_intent, f_entities, f_constraints, f_urgency, f_steps, pred_dict):
    base = (f_intent + f_entities + f_constraints + f_urgency + f_steps) / 5.0
    has_all = isinstance(pred_dict, dict) and all(k in pred_dict for k in FIELDS)
    bonus = BONUS_ALL_FIELDS if has_all else 0.0
    return min(base + bonus, 1.0)

def shaped_reward_from_fields(f_intent, f_entities, f_constraints, f_urgency, f_steps, pred_dict):
    weighted = (f_intent + f_entities + f_constraints + f_urgency + STEP_WEIGHT * f_steps) / (4.0 + STEP_WEIGHT)
    has_all = isinstance(pred_dict, dict) and all(k in pred_dict for k in FIELDS)
    bonus = BONUS_ALL_FIELDS if has_all else 0.0
    return min(weighted + bonus, 1.0)
    ...
    if not os.path.exists(CSV_LOG):
        with open(CSV_LOG, "w", newline="", encoding="utf-8") as f:
            csv.writer(f).writerow([
                ...
                "reward_base","reward_base_per_1k","reward_shaped","reward_shaped_per_1k",
...
def log_row(**kw):
    with open(CSV_LOG, "a", newline="", encoding="utf-8") as f:
        csv.writer(f).writerow([
            ...
            kw["input"], kw["reference"], kw["output"], kw["tokens"],
            kw["reward_base"], kw["reward_base_per_1k"], kw["reward_shaped"], kw["reward_shaped_per_1k"],
...
def main():
    ...
    for itr in range(1, ITERATIONS+1):
            ...
            if pass1_valid:
                ...
                if not cands:
                    ...
                    if isinstance(j2b, dict) and "steps" in j2b:
                        ...
                        if sc_b > 0:
                            cands.append((sc_b, steps_b, toks2b))
        ...
        if pred is None:
            ...
            reward_base = reward_shaped = 0.0
        else:
            json_valid = 1.0
            ref = json.loads(ref_str)
            f1_intent, f1_entities, f1_constraints, f1_urgency, f1_steps = score_fields(pred, ref)
            reward_base   = base_reward_from_fields(f1_intent, f1_entities, f1_constraints, f1_urgency, f1_steps, pred)
            reward_shaped = shaped_reward_from_fields(f1_intent, f1_entities, f1_constraints, f1_urgency, f1_steps, pred)

        reward_base_per_1k   = (reward_base   / max(1, toks)) * 1000.0
        reward_shaped_per_1k = (reward_shaped / max(1, toks)) * 1000.0

        # bandit update uses SHAPED reward
        bandit.update(arm, reward_shaped)
        ...
        log_row(
            ...
            reward_base=round(reward_base,6), reward_base_per_1k=round(reward_base_per_1k,6),
            reward_shaped=round(reward_shaped,6), reward_shaped_per_1k=round(reward_shaped_per_1k,6),
        ...
        if itr % 10 == 0:
            ...
            print(f"[iter {itr}] best_avg≈{best_avg:.3f} (STEP_WEIGHT={STEP_WEIGHT}) | router_mode={router_mode}")
