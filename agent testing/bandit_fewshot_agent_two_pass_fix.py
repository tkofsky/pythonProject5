...
def call_llm(messages, temp, max_tokens=300):
    resp = client.chat.completions.create(model=GEN_MODEL, messages=messages, temperature=temp, max_tokens=max_tokens)
...
# Two-pass prompts
PASS1_SYS="Return ONLY JSON with the exact keys: intent, entities, constraints, urgency. Do NOT include steps. If a value is unknown, set it to null. No extra keys. No commentary."
PASS2_SYS="Given the structured fields below, output ONLY the JSON array for \"steps\" (3–5 concise action verbs/phrases). Do NOT include any other keys or text. Do NOT modify the provided fields."

# Steps few-shot mini-demos (for Pass 2)
STEPS_DEMOS = [
    {
        "fields": {
            "intent": "book_train",
    ...
    },
    {
        "fields": {
            ...
            "entities": {"participants":["Maya"],"time_window":"next Tuesday after 3pm","location":"Zoom"},
...
def steps_fewshot_block(n=1):
    n = max(0, min(n, len(STEPS_DEMOS)))
    lines = []
    for demo in STEPS_DEMOS[:n]:
        lines.append("FIELDS: " + json.dumps(demo["fields"], ensure_ascii=False))
        lines.append('STEPS: ' + json.dumps(demo["steps"], ensure_ascii=False))
    return "\n".join(lines) + ("\n" if lines else "")

def run_two_pass(user_text, temp):
    # Pass 1: extract slots (no steps)
    msgs1 = [
        {"role":"system","content":PASS1_SYS},
        {"role":"user","content":user_text}
    ]
    out1, toks1 = call_llm(msgs1, temp, max_tokens=350)
    slots = safe_json(out1)
    if slots is None or not isinstance(slots, dict):
        # invalid; pass reward calc will yield 0
        return out1, toks1

    # Pass 2: generate ONLY steps array (use a tiny steps few-shot to bias style)
    payload = json.dumps(slots, ensure_ascii=False)
    pass2_user = (
        steps_fewshot_block(1) +
        "Structured fields (do not alter):\n" + payload + "\n\n"
        "Now return ONLY the JSON array for 'steps' (3–5 items)."
    )
    msgs2 = [
        {"role":"system","content":PASS2_SYS},
        {"role":"user","content": pass2_user}
    ]
    out2, toks2 = call_llm(msgs2, temp, max_tokens=450)

    # Parse steps
    steps = safe_json(out2)
    # Accept either a bare array or an object with "steps"
    if isinstance(steps, dict) and "steps" in steps:
        steps = steps["steps"]
    # Retry once if not a proper list
    if not isinstance(steps, list) or len(steps) == 0:
        retry_user = "Return ONLY the JSON array for 'steps' with 3–5 concise items. No other keys."
        out2b, toks2b = call_llm(
            [{"role":"system","content":PASS2_SYS}, {"role":"user","content": retry_user}], temp, max_tokens=200
        )
        steps = safe_json(out2b)
        if isinstance(steps, dict) and "steps" in steps:
            steps = steps["steps"]
        toks2 += toks2b

    if not isinstance(steps, list):
        steps = []  # reward will reflect weakness in f1_steps

    # Merge final JSON
    final_json = {**slots, "steps": steps}
    out_text = json.dumps(final_json, ensure_ascii=False)
    return out_text, (toks1 + toks2)
...
def main():
    ...
    for itr in range(1,ITERATIONS+1):
        ...
        if itr%10==0:
            best_avg = max(
                (totals) / max(1, counts)
                for (totals), counts in zip(bandit.totals.values(), bandit.counts.values())
