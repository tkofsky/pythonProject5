...
Logs trials to CSV with timestamps, latency, and F1-style metrics.
"""
...
CSV_LOG      = "bandit_log_ucb1_dataset_shots_f1.csv"
...
def score_output(example: Dict[str, Any], text: str) -> Dict[str, float]:
    """
    Placeholder scoring:
    - Attempts to parse JSON.
    - F1_* metrics set to 0.0 by default.
    - reward_overall derived from simple structural heuristic.
    """
    try:
    ...
    base_score = simple_structure_score(text)
    ...
    reward_overall = base_score

    return {
        "reward_overall": reward_overall,
        "F1_intent": F1_intent,
        "F1_entities": F1_entities,
        "F1_constraints": F1_constraints,
        "F1_urgency": F1_urgency,
        "F1_steps": F1_steps,
    }
...
def run_trial_with_variant(
    ...
    try:
        ...
        scores = score_output(current, output)
...
def main():
    ...
    for t in range(n_trials):
        ...
        shots = variant.get("shots", 0)
        mutation = variant.get("mutation", "base")
        ...
        print(
            ...
            f"arm={arm} ({variant['id']}, mut={mutation}, shots={shots}) "
    ...
    for i, v in enumerate(variants):
        print(
            f"  Arm {i} ({v['id']} | {v['category']} | mut={v.get('mutation', 'base')} | shots={v.get('shots', 0)}): "
