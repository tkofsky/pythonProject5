...
UCB_C        = 1.0
N_TRIALS     = 200
COST_LAMBDA  = 0.3   # cost per second of latency
...
# -------- Arms: template x shots x passes x temperature --------
...
def run_trial(cfg: Dict[str, Any], task_i: int) -> Dict[str, Any]:
    ...
    total_latency = r1["latency"]
    total_prompt_len = len(p1)
    ...
    if passes == 2 and not err:
        ...
        total_latency += r2["latency"]
        total_prompt_len += len(p2)
        total_in_tokens += r2["input_tokens"]
        total_out_tokens += r2["output_tokens"]
    ...
    adj_reward = raw_reward - COST_LAMBDA * total_latency
...
def main():
    ...
    for t in range(1, N_TRIALS + 1):
        ...
        sum_total_tok[arm]  += res["total_tokens"]
        ...
        print(
            ...
            f"Qadj={q_adj[arm]:.3f} n={pulls[arm]} "
    ...
    for i, cfg in enumerate(ARMS):
        if pulls[i] > 0:
            ...
            mean_tok = sum_total_tok[i] / pulls[i]
        else:
            lcb = ucb = bw = 0.0
            conv = False
            mean_raw = 0.0
