...
CSV_LOG      = "bandit_log_ucb1_dataset_shots_f1_real.csv"
...
def flatten_to_items(value: Any) -> List[str]:
    ...
    def _walk(v: Any):
        ...
        if isinstance(v, (str, int, float, bool)):
            ...
            # Use values only to avoid key noise
            for val in v.values():
                _walk(val)
        else:
            items.append(str(v).strip().lower())
...
def score_output(example: Dict[str, Any], text: str) -> Dict[str, float]:
    target_str = get_example_output(example)
    if not target_str:
    ...
    try:
        ...
        if not isinstance(target_json, dict) or not isinstance(pred_json, dict):
    ...
    for field in fields:
        true_val = target_json.get(field, None)
        ...
        f1_scores[field] = f1
        # Only include fields in reward if there is at least some ground truth signal
        if true_items:
            f1_values.append(f1)
...
def main():
    ...
    for i, v in enumerate(variants):
        print(
            ...
            f"mut={v.get('mutation', 'base')} | shots={v.get('shots', 0)}): "
