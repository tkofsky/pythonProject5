...
# Paths
DATA_PATH = "extended_sample_dataset.json"
...
# Semantic similarity reward using embeddings
def calculate_reward(output, reference):
    emb_output = client.embeddings.create(model="text-embedding-3-small", input=output).data[0].embedding
    emb_reference = client.embeddings.create(model="text-embedding-3-small", input=reference).data[0].embedding
    similarity = cosine_similarity([emb_output], [emb_reference])[0][0]
    return similarity  # Range: 0 to 1
