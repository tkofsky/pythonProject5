...
model = SentenceTransformer('paraphrase-MiniLM-L6-v2')  # Consider using models fine-tuned for similarity tasks
...
def generate_answer(context, question):
    ...
    """
    try:
        # Enhance the prompt to focus on extracting specific information
        prompt = (
            f"Context: {context}\n\n"
            f"Focus specifically on details about Stephen Curry's height.\n\n"
            f"Question: {question}\n\nAnswer:"
        )
        ...
        response = openai.chat.completions.create(
            ...
            max_tokens=150,
            temperature=.2  # Low temperature to make answers more focused and factual
        )
...
def adjust_score(score, threshold=0.8):
    """
    Adjusts the score based on a threshold. Scores close to 1 are boosted, while scores below the threshold are penalized.
    """
    if score > threshold:
        return min(1.0, score + (1.0 - score) * 0.5)  # Boost scores close to 1
    else:
        return max(0, score - (threshold - score) * 0.5)  # Penalize scores far from 1
...
def score_answer_against_chunk(chunk, answer):
    ...
    adjusted_score = adjust_score(normalized_score)
...
def main():
    ...
    question = "what is steph curry's height?"
